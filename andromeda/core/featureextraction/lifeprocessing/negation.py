...
class Negation:
    ...
    def process(self, token_list, token_feature_class):
        ...
        for idx, feature_dict in enumerate(token_list):
            ...
            token_pos_tag = TokenListUtil.get_pos_tag(token_class, idx)

            # Get the preceding token
            next_token = TokenListUtil.get_token(token_list, idx+1, default='')
            ...
            if do_negate and token_class in Constants.LIFE__NEGATED_WORD_TOKEN_CLASSES:
    ...
    def _do_negate(self, do_negate, token, token_class, token_pos_tag, scope_size):
        ...
        if scope_size >= Constants.LIFE__NEGATION_MAX_SCOPE_SIZE:
            return False

        # If token is "context-splitting" (e.g., punctuation mark, url, email, hashtag), stop negating
        if token_class in Constants.LIFE__NEGATION_BOUNDARY_TOKEN_CLASSES or token_pos_tag in Constants.LIFE__NEGATION_BOUNDARY_POS_TAGS:
            if token_class not in Constants.LIFE__NEGATION_BOUNDARY_TOKEN_EXCEPTION_CLASSES:
                return False
