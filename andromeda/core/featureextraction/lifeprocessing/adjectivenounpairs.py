...
class AdjectiveNounPairs:
    ...
    def process(token_list, token_feature_class):
        ...
        pos_tags_string = LifeBase.calculate_pos_tag_string(token_list,
                                                            wildcard_pos_tags_list=LifeConstants.ANP_RULE__POS_WILDCARD_TOKEN_CLASSES,
                                                            unknown_pos_tags_list=LifeConstants.ANP_RULE__POS_UNKNOWN_TOKEN_CLASSES)
    ...
    def _process_premodifiers(token_list, pos_tag_string):
        ...
        for m in p.finditer(pos_tag_string):
            ...
            for pos in range(m.start(1), m.end(1)):
                if pos_tag_string[pos] in [POSTagger.POS_TAG__ADJECTIVE]:
            ...
            for pos in range(m.start(4), m.end(4)):
                if pos_tag_string[pos] in [POSTagger.POS_TAG__COMMON_NOUN]:
    ...
    def _set_anp(token_list, adjective_pos_list, noun_pos_list):
        ...
        for adjective_pos in adjective_pos_list:
            if LifeConstants.LIFE__ANP not in token_list[adjective_pos][LifeConstants.TOKEN_FEATURE__BASE]:
                token_list[adjective_pos][LifeConstants.TOKEN_FEATURE__BASE][LifeConstants.LIFE__ANP] = []
            token_list[adjective_pos][LifeConstants.TOKEN_FEATURE__BASE][LifeConstants.LIFE__ANP].extend(noun_pos_list)
        # Assign the position of the describing adjectives to each noun
        for noun_pos in noun_pos_list:
            if LifeConstants.LIFE__ANP not in token_list[noun_pos][LifeConstants.TOKEN_FEATURE__BASE]:
                token_list[noun_pos][LifeConstants.TOKEN_FEATURE__BASE][LifeConstants.LIFE__ANP] = []
            token_list[noun_pos][LifeConstants.TOKEN_FEATURE__BASE][LifeConstants.LIFE__ANP].extend(adjective_pos_list)
